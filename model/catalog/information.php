...
class ModelCatalogInformation extends Model {
	public function addInformation($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', type = '" . (int)$data['type'] . "', department = '" . (int)$data['department'] . "', category = '" . (int)$data['category'] . "', related_article = '" . $data['related_article'] . "', related_product = '" . $data['related_product'] . "', image = '" .$data['image'] . "', product_id = '" . (int)$data['product_id'] . "'");
        ...
        if (isset($data['product'])) {
            foreach ($data['product'] as $product) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "information_product SET product_id = '" . (int)$product . "', information_id = '" . (int)$information_id . "'");
	...
	public function editInformation($information_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "information SET sort_order = '" . (int)$data['sort_order'] . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', status = '" . (int)$data['status'] . "', type = '" . (int)$data['type'] . "', department = '" . (int)$data['department'] . "', category = '" . (int)$data['category'] . "', related_article = '" .$data['related_article'] . "', related_product = '" .$data['related_product'] . "', image = '" .$data['image'] . "', product_id = '" . (int)$data['product_id'] . "' WHERE information_id = '" . (int)$information_id . "'");
        ...
        if (isset($data['product']))
        {
            $this->db->query("DELETE FROM " . DB_PREFIX . "information_product WHERE information_id = '" . (int)$information_id . "'");
            foreach ($data['product'] as $product) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "information_product SET product_id = '" . (int)$product . "', information_id = '" . (int)$information_id . "'");
    ...
    public function getInformationProducts($information_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information_product WHERE information_id = '" . (int)$information_id . "'");
        return $query->rows;
    }


    public function getBlogDepartment() {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_category   WHERE parent_id =0");
        return $query->rows;
    }


    public function getBlogCategory($id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_category   WHERE parent_id ='".$id."'");
        return $query->rows;
    }
