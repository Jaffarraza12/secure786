...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['depart_id'])) {
            ...
            $data['depart_id'] = $category_info['depart_id'];
        ...
        if (isset($this->request->post['product_category'])) {
            ...
            $categories = $this->model_catalog_product->getCategoryCategories($this->request->get['category_id']);
        ...
        if (isset($this->request->post['size_chart'])) {
            $data['size_chart'] = $this->request->post['size_chart'];
        } elseif (!empty($category_info)) {
            $data['size_chart'] = $category_info['size_chart'];
        } else {
            $data['size_chart'] = '';
        }

        if (isset($this->request->post['size_chart']) && is_file(DIR_IMAGE . $this->request->post['size_chart'])) {
            $data['size_chart_thumb'] = $this->model_tool_image->resize($this->request->post['size_chart'], 100, 100);
        } elseif (!empty($category_info) && is_file(DIR_IMAGE . $category_info['size_chart'])) {
            $data['size_chart_thumb'] = $this->model_tool_image->resize($category_info['size_chart'], 100, 100);
        } else {
            $data['size_chart_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
        }
		...
		$data['category_content'] = $this->model_catalog_category->getCategoryContent($this->request->get['category_id']);

        if (isset($this->request->post['category_question'])) {
            $data['category_question'] = $this->request->post['category_question'];
        } elseif (isset($this->request->get['category_id'])) {
            $data['category_question'] = array();
        } else {
            $data['category_question'] = array();
        }

        if (isset($this->request->post['category_answer'])) {
            $data['category_answer'] = $this->request->post['category_answer'];
        } elseif (isset($this->request->get['category_id'])) {
            $data['category_answer'] = array();
        } else {
            $data['category_answer'] = array();
        }
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$filter_data = array(
				...
				'depart_id' => $this->request->get['did'],
				'sort'        => 'name',
				'order'       => 'ASC',
				'start'       => 0,
				'limit'       => 5
